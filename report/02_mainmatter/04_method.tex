...
Another detail is that LingoDB's newer versions contain numerous
features and optimisations that are not relevant to us, so to simplify implementation
...
by a handwritten analyser for whether the query can be executed, and then parsed.
These hand written components are annotated in light-peach.
...
needed to be passed through to C++. This causes a design decision. Good practice
...
This means if we run without being inside of PostgreSQL, no psql libraries can
...
workarounds that were implemented were for decimals and the various types of strings.
For decimals, \texttt{i128} is enough precision for most of the TPC-H tests, and is what
...
changes across the DB, DSA, RelAlg, and Util dialects. The changes were primarily
API compatibility updates with minimal semantic modifications: 94 operations total
across all dialects (93 in LingoDB, 94 in PGX), 21 types (identical count), and
mostly LLVM 20 API compatibility updates.
...
While the query analyser is fully written, in the final state it forces all
queries through to pgx-lower for testing. This enables testing new things and
where they fail. It functions by doing a depth-first-search through the plan
tree and validating the nodes are supported by the engine at this point, however,
in the future there can be work here for deciding whether a query is worth
running based on its cost metrics as well.
...
hash table implementations to rely on the PostgreSQL API rather than standard
collections.
...
before the node is used and contains initialising variables such as the parameters,
...
which is also not utilised, group columns, number of groups, it can produce
...
NaN/nullable), this introduces implementation pains.
...
sort operations, and scalar maths. With these casts, the SQLTypeInference object
supports upcasting such that if two types have an operation, they are both
first cast into the larger datatype, such as \texttt{i16} + \texttt{i32}, then the smaller one will
both be cast into \texttt{i32} first.
...
InstCombine simplifies instructions through algebraic transformations; PromotePass
elevates memory operations to register operations; LICM (Loop Invariant Code Motion)
moves loop-independent code outside of loops by hoisting to preheaders or sinking
to exit blocks; the reassociation pass reorders expressions to enable further
optimisations; and GVN (Global Value Numbering) eliminates redundant computations
by identifying values that must be equal.
...
with an i5-6500T, 16 GB of RAM and a Samsung MZVLB256HAHQ-000L7 NVMe disk.
...
React for the frontend, and Docker containers to support the reverse proxy

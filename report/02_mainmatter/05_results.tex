...
\begin{figure}[H]
	...
	\caption{PostgreSQL TPC-H query 20 indexes enabled at SF = 0.16. 15 minutes
		runtime}
...
\begin{figure}[H]
	...
	\caption{pgx-lower TPC-H query 20 indexes enabled at SF = 0.16. 1.18 seconds
		runtime}
...
% TODO: I also need to make the Umbra history clearer and what features it has
Since replacing PostgreSQL's execution engine with a JIT-focused one is quite
generic, there are a number of directions future works can take. The system
itself and be improved by fully implementing the plan nodes, and it
...
execution engines to integrate in can be considered.

To progress this system further and make it a full extension, the plan tree
nodes need to be fully implemented, and the query analyzer can be improved.
...
and clearer ways to use the PostgreSQL API. Namingly, pre-compiling
...
and Umbra's caching system. Most of these are only applicable inside a
LLVM/MLIR system though, and if WebAssembly is used instead a lot of
them can be skipped. More generically, parallelism can be improved,
JIT compilation tuning, cross-platform support, subquery
deduplication, and further tuning.

In databasing, and research broadly, it is vital to keep in mind whether the
research is useful and impactful, as per Michael Stonebraker's fear that
the field is becoming polishing a round ball~\cite{top-ten-fears}. For
...
systems will usually move away from PostgreSQL and into a more scaleable
...
needs. It also contains a query optimisation engine, which is unnecessary
...
depending on the analyzer's rules.
...
% TODO: Is the signing off note I want to do? 
...
and inserting it into PostgreSQL as drop-in engine replacement.

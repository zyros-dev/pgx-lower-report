...
Structured Query Language (SQL) is parsed, turned to RA (relational-algebra),
...
For non-compiler databases, a volcano operator model tree is used, such as
Figure~\ref{fig:execution_tree} \cite{long-masters-thesis}. The root node has a \texttt{produce()} function
which calls its children's \texttt{produce()}, until it calls a leaf node,
...
caches are barely used. An i5-570, a popular CPU in 2010 had an 8 megabyte L3 cache,
but in 2024 an i5-14600K has a 24 megabyte L3 cache \cite{passmark_i5760},
...
\cite{seagate_barracuda_7200_12} \cite{samsung_990_pro}. Such dramatic increases
could mean the algorithms can fundamentally change.

This has led to the vectorized execution model and
...
pick a relational database. Atomicity means transactions are a single unit
...
due to the era in which these systems were developed, when disks were much slower and
...
the optimiser rather than in runtime, and more \cite{inersjo2021}. However, despite
these advances, query optimisers frequently produce suboptimal plans \cite{optimizer-bad-1,optimizer-bad-2}. Another essential pattern involves genetic algorithms
...
such as eight kilobytes. This is loaded into a buffer pool object which holds it in RAM. The operating system or environment manages where this memory goes
...
strategies depending on the situation, such as last-recently-used, most-recently-used, and more, which
...
hybrid architecture that supports both OLTP and OLAP operations
\cite{herrera2021hybrid}. There is currently debate about whether
this hybrid architecture is still useful, as putting pressure
on a database serving users commonly causes reliability issues
\cite{mooncake_htap}.
...
can run the primary program, then dedicate background threads to
improving code optimisation and swap in the optimized
version when ready \cite{hyper-compiler-2}. This provides benefits such as faster initial
compilation and faster development cycles.
...
measured that their JIT solution's speculative optimisations can lead up to $50\%$ performance gains \cite{azul_jit_aot}.

Evaluating a good branch prediction value is difficult,
but a reasonable baseline is that $1\%$ misprediction rate is too high and should be optimised in
a low latency environment \cite{farrier2025branch}. This is not a formal
standard, but rather based on empirical knowledge \cite{branch-prediction-values}. Depending on the
CPU, a branch mispredict can cost between 10 and 35 CPU cycles, with a
safe range being 14-25 cycles. That is, with
1 branch every 10 instructions, a $5\%$ misprediction rate,
and a 20 cycle penalty per misprediction, approximately $10\%$ of the runtime
will be spent resolving mispredictions \cite{eyerman2006branch}.
...
itself, they have EXP support using \texttt{llvm-jit}. A variety of research databases
...
tightly coupled with the LLVM project~\cite{mlir-paper}. It provides a framework to define custom dialects and progressively lower them to machine code. A key benefit is that developers can define high-level dialects that others can target and build upon.
...
When evaluating benchmark results, the \textit{coefficient of variation} (CV)
will be used, and it is a standardized measure of dispersion that expresses
the standard deviation as a percentage of the mean~\cite{stats-libretexts-cv}.
It is calculated as
...
useful when comparing the variability of datasets with different units or scales,
as it provides a unitless measure that enables relative comparison of
measurement precision across different benchmarking configurations.

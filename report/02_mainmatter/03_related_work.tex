...
We summarise relevant works in the compiled query space and their architectures. Compiled query engines originally dominated the database industry \cite{system-r}, but
...
PostgreSQL's extension system is covered in Section~\ref{sec:postgresql-related}, followed by System R
in Section~\ref{sec:system-r} as a classical example. HyPer and Umbra (Sections~\ref{sec:hyper} and~\ref{sec:umbra})
re-introduced compilation in modern databases, while Mutable (Section~\ref{sec:mutable}) and LingoDB
(Section~\ref{sec:lingodb}) provide research examples. PostgreSQL's JIT compilation and prior attempts conclude the survey.
...
for expressions (EXP) \cite{clickhouse}. The Carnegie Mellon Database group created NoisePage, a columnar, in-memory system
with full query expression compilation (QEP), with a single node. They targeted
ML-driven self optimisation in their research, but the project archived in February 21, 2023 \cite{noisepage}. DuckDB is marketed
...
extensions is well-established. Three such examples are Citus~\cite{citus},
...
PostgreSQL in 2017 \cite{postgres-pr-1,postgres-pr-2}. Developers expressed doubts about adding full query compilation support, with concerns that
...
expressions \cite{psql-jit-release-plan}. Most CPU time occurs in expression components
(such as y > 8 in SELECT * from table WHERE x > 8;). Furthermore,
tuple deformation provides significant benefits as it interacts with the
...
were too low. Despite this concern, PostgreSQL version 11 released with JIT disabled by default.
...
Full JIT implementation was preceded by profiling work showing different TPC-H benchmarks
pressure different nodes (Figure~\ref{fig:pgcon-profiling}), enabling informed decisions about optimization priorities.
...
Other database systems also support extensions, and many systems
...
Tableau purchased in 2016 for production use \cite{tableau-hyper}, proving that
...
the source code is not accessible, though a binary is available on their website for benchmarking.
...
inlining them into the LLVM IR. LLVM's JIT executor then executes the IR. By utilising LLVM IR, they can take advantage of overflow flags and
...
measurement for how long live queries are taking. The
...
in a \emph{work-stealing} stage they log how long the job took. With a combination
...
multi-threaded compilation/execution created a viable JIT compiled-query application. A primary criticism: they effectively
...
they mostly address compiler latency.

%-------------------------------------------------------------------------------- 
\section{Umbra}\label{sec:umbra}
%-------------------------------------------------------------------------------- 
Umbra, created in 2020 by Thomas Neumann (HyPer's creator), demonstrates that HyPer's in-memory
concepts apply to on-disk systems \cite{umbra}. Recent SSD improvements and buffer management advances made
this possible. Umbra integrates LeanStore concepts for
...
times; Umbra's approach of invoking the compiler multiple times during execution
...
but direct JIT compiler access enabling adaptive compilation for optimisation choice changes provides distinct advantages. Additionally, Umbra supports
...
during execution. HyPer's direct bytecode interpreter
achieves lower execution startup times.
...
Results show lower performance than HyPer but better than
DuckDB \cite{lingo-db-1}. Performance was not the primary focus; rather, implementing standard optimisation patterns within the compiler was key. Notably, LingoDB uses approximately 10,000 lines of code for query execution model,
while Mutable uses 22,944 lines despite skipping query
optimization. Comparisons show LingoDB uses three times
...
visible in Figure~\ref{fig:created-benchmark-results}. The benchmarks used TPC-H as most
...
The analysis used \texttt{perf} on PostgreSQL during TPC-H queries 1, 3, 6, 12 and 14 in
...
been shown. Accomplishing this shows a way to rely on previous
